...
if access_code == '':
    ...
    credentials = GoogleCredentials(access_token, client_id, client_secret,
                                    refresh_token, 3920, 'https://accounts.google.com/o/oauth2/token', 'test')
    http = httplib2.Http()
    http = credentials.authorize(http)
    service = build('analytics', 'v4', http=http)
    # Printing is helpful to know that the service was created.
    # Not necessary to print unless wanting visual confirmation.
    #print(service)
